...
def my_canny(img, min_val, max_val, sobel_size=3, is_L2_gradient=True):
    ...
    for y in range(1, edge_gradient.shape[0] - 1):
        for x in range(1, edge_gradient.shape[1] - 1):
            ...
            local_grad_maximum = area_grad_intensity[1, 1]

            if current_angle == 0:
                if local_grad_maximum > max(area_grad_intensity[1, 0], area_grad_intensity[1, 2]):
                    keep_mask[y, x] = 255
                else:
                    edge_gradient[y, x] = 0
            elif current_angle == 45:
                if local_grad_maximum > max(area_grad_intensity[2, 0], area_grad_intensity[0, 2]):
                    keep_mask[y, x] = 255
                else:
                    edge_gradient[y, x] = 0
            elif current_angle == 90:
                if local_grad_maximum > max(area_grad_intensity[0, 1], area_grad_intensity[2, 1]):
                    keep_mask[y, x] = 255
                else:
                    edge_gradient[y, x] = 0
            elif current_angle == 135:
                if local_grad_maximum > max(area_grad_intensity[0, 0], area_grad_intensity[2, 2]):
                    keep_mask[y, x] = 255
                else:
                    edge_gradient[y, x] = 0
    ...
    # canny_mask[(keep_mask > 0) * (edge_gradient > int((min_val + max_val) / 3))] = 255

    return canny_mask


img = cv2.imread('/home/quan/PycharmProjects/OpenCV_practical/download.jpeg')
img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
my_canny = my_canny(img, min_val=100, max_val=200)
edges = cv2.Canny(img, 100, 200)

cv2.imshow('my_canny', my_canny)
cv2.imshow('cv2', edges)
cv2.waitKey(0)